...
void ActivitiesModel::init()
{
    ...
    if (!q.exec(QStringLiteral("SELECT a.id, a.name, a.minrepeats, a.maxrepeats, a.distance, a.category, c.name as categoryname, c.color, (SELECT COUNT(id) FROM records WHERE activity = a.id) AS records, (SELECT COUNT(id) FROM activities WHERE category = a.category) AS catActs, a.sensor FROM activities a JOIN categories c ON c.id = a.category"))) {
        setInOperation(false);
        return;
    }
    ...
    while (q.next()) {
        Activity *a = new Activity(q.value(0).toInt(), q.value(1).toString(), q.value(2).toInt(), q.value(3).toInt(), q.value(4).toBool(), q.value(8).toInt(), q.value(10).toInt(), this);
 ...
 */
void ActivitiesModel::add(int databaseId, const QString &name, Category *c, int minRepeats, int maxRepeats, bool distance, int sensorType)
{
    Activity *a = new Activity(databaseId, name, minRepeats, maxRepeats, distance, 0, sensorType, this);
    c->setParent(a);
    a->setCategory(c);
